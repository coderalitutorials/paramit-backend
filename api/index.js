...
app.post("/api/contact", async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      // 1. Sender name set kar diya "Paramit Services" 👇
      from: `"Paramit Services" <${process.env.EMAIL_USER}>`,
      to: process.env.RECEIVER_EMAIL,
      replyTo: email,
      // 2. Subject prefix update kar diya 👇
      subject: `[Paramit Services] 🚨 New Lead: ${service || "Contact Form Submission"}`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        <body style="margin: 0; padding: 40px 10px; background-color: #0f172a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          <div class="email-container" style="max-width: 500px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.4); border: 1px solid #334155;">
            ...
            <div class="content-wrapper" style="padding: 30px 25px; color: #cbd5e1;">
              <p style="margin: 0 0 10px 0; font-size: 16px; font-weight: 600; color: #ffffff;">Hello Paramit Services Team,</p>
              <p style="margin: 0 0 25px 0; font-size: 14px; color: #94a3b8;">A new enquiry has been submitted through your website.</p>
              
              <div style="background-color: #0f172a; padding: 14px 18px; margin-bottom: 10px; border-radius: 6px; border: 1px solid #334155;">
                <table style="width: 100%; border-collapse: collapse;">
                  <tr><td style="color: #94a3b8; font-size: 14px; width: 35%;">Name</td><td style="color: #ffffff; font-weight: 600; text-align: right;">${name}</td></tr>
                </table>
              </div>

              <div style="background-color: #0f172a; padding: 14px 18px; margin-bottom: 10px; border-radius: 6px; border: 1px solid #334155;">
                <table style="width: 100%; border-collapse: collapse;">
                  <tr><td style="color: #94a3b8; font-size: 14px; width: 35%;">Email</td><td style="color: #38bdf8; font-weight: 600; text-align: right;">${email}</td></tr>
                </table>
              </div>

              <div style="background-color: #0f172a; padding: 14px 18px; margin-bottom: 10px; border-radius: 6px; border: 1px solid #334155;">
                <table style="width: 100%; border-collapse: collapse;">
                  <tr><td style="color: #94a3b8; font-size: 14px; width: 35%;">Phone</td><td style="color: #ffffff; font-weight: 600; text-align: right;">${phone}</td></tr>
                </table>
              </div>

              <div style="background-color: #0f172a; padding: 14px 18px; margin-bottom: 24px; border-radius: 6px; border: 1px solid #334155;">
                <table style="width: 100%; border-collapse: collapse;">
                  <tr><td style="color: #94a3b8; font-size: 14px; width: 35%;">Service</td><td style="color: #34d399; font-weight: 700; text-align: right;">${service || "Not selected"}</td></tr>
                </table>
              </div>

              <div style="background-color: #0f172a; border: 1px solid #334155; border-radius: 8px; padding: 16px; font-size: 14px; color: #cbd5e1; font-style: italic;">
                "${message}"
              </div>
            </div>
            <div style="padding: 20px; background-color: #0f172a; text-align: center; border-top: 1px solid #334155;">
              <p style="margin: 0; font-size: 11px; color: #475569;">Automated notification for Paramit Services.</p>
            </div>
          </div>
        </body>
        </html>
...
app.post("/api/callback", async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      // 1. Sender name "Paramit Services" yahan bhi set kar diya 👇
      from: `"Paramit Services" <${process.env.EMAIL_USER}>`,
      to: process.env.RECEIVER_EMAIL,
      // 2. Subject prefix update kar diya 👇
      subject: `[Paramit Services Callback] 📞 Quick Request from ${name}`,
      html: `
        <body style="margin: 0; padding: 40px 10px; background-color: #0f172a; font-family: sans-serif;">
          <div style="max-width: 500px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; overflow: hidden; border: 1px solid #334155;">
            <div style="background: linear-gradient(135deg, #801E59 0%, #5c123f 100%); padding: 35px 20px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff;">Call Back Requested</h1>
            </div>
            ...
            <div style="padding: 15px; background-color: #0f172a; text-align: center; border-top: 1px solid #334155;">
              <p style="font-size: 11px; color: #475569;">Automated call-back system for Paramit Services.</p>
            </div>
          </div>
        </body>
      `,
    ...
    res.status(200).json({ success: true, message: "Call back request sent!" });
  } catch (error) {
    res.status(500).json({ success: false, message: "Email sending failed" });
  }
});
